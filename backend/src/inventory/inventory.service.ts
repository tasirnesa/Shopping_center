...
export class InventoryService {
    ...
    async createPurchase(dto: CreatePurchaseDto) {
        ...
        return this.prisma.$transaction(async (tx) => {
            // 1. Create Purchase header + details
            ...
            // 2. Update stock balances and log transactions
    ...
    async getStockBalance(branchId?: string) {
    ...
    async adjustStock(dto: AdjustStockDto) {
        return this.prisma.$transaction(async (tx) => {
            // Validate current stock when reducing
    ...
    async transferStock(dto: TransferStockDto) {
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            // Log IN transaction (destination)
            await tx.inventoryTransaction.create({
                data: {
                    productId: dto.productId,
