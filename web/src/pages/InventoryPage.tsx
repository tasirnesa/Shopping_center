  ...
  return (
    <Box sx={{ pb: { xs: 8, md: 2 } }}>
      ...
      >
        ...
        >
          <Tab
            icon={<Inventory2 sx={{ mb: "0 !important", mr: 1 }} />}
            iconPosition="start"
            label="Stock"
          />
          <Tab
            icon={<ReceiptLong sx={{ mb: "0 !important", mr: 1 }} />}
            iconPosition="start"
            label="Receipt"
          />
          <Tab
            icon={<TuneRounded sx={{ mb: "0 !important", mr: 1 }} />}
            iconPosition="start"
            label="Adjust"
          />
          <Tab
            icon={<SwapHoriz sx={{ mb: "0 !important", mr: 1 }} />}
            iconPosition="start"
            label="Transfer"
          />
          <Tab label="History" />
...
function StockBalanceTab() {
  ...
  useEffect(() => {
    api.get("/inventory/stock-balance").then((r) => {
      setData(r.data);
      setLoading(false);
    });
  }, []);
        ...
        <EmptyState
          ...
          description={
            search
              ...
              : "Receive goods or adjust stock to see balances here."
          }
        />
      ) : isMobile ? (
        // Mobile Card View
                ...
                <CardContent sx={{ p: "16px !important" }}>
                  ...
                  <Stack direction="row" spacing={1} mb={2}>
                    ...
                    <Chip
                      ...
                      label={s.product.category?.name || "Uncategorized"}
                      size="small"
                      variant="outlined"
                      sx={{ borderRadius: 2 }}
                    />
...
function GoodsReceiptTab() {
  ...
  const handleAddLine = () =>
    setLines([...lines, { productId: "", quantity: 1, cost: 0 }]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSupplierId("");
      setBranchId("");
      setLines([{ productId: "", quantity: 1, cost: 0 }]);
      load();
    } catch (e: any) {
            ...
            <TableBody>
              {purchases.map((p: any) => (
                <TableRow key={p.id} hover>
                  ...
                  <TableCell sx={{ fontWeight: 600 }}>{p.supplier?.name}</TableCell>
                  <TableCell>
      ...
      >
        ...
        <DialogContent>
          ...
          <TextField
            ...
            onChange={(e) =>
              setNewSupp({ ...newSupp, contact: e.target.value })
            }
          />
          <TextField
            label="Email"
        ...
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={() => setSuppOpen(false)} sx={{ borderRadius: 2 }}>
            Cancel
          </Button>
          <Button
            variant="contained"
...
function AdjustmentTab() {
  ...
  useEffect(() => {
    Promise.all([api.get("/products"), api.get("/foundation/branches")]).then(
      ([p, b]) => {
        setProducts(p.data);
        setBranches(b.data);
      },
...
function MovementLogTab() {
  ...
  useEffect(() => {
    api.get("/inventory/transactions").then((r) => {
      setData(r.data);
      setLoading(false);
    });
  }, []);
                ...
                <CardContent sx={{ p: "16px !important" }}>
                  ...
                  >
                    ...
                    <Typography
                      variant="h6"
                      sx={{ fontWeight: 800 }}
                      color={t.type === "IN" ? "success.main" : "error.main"}
                    >
                      {t.type === "IN" ? "+" : "-"}
                      {t.quantity}
                    </Typography>
